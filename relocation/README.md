# D28 Relocation — Authoritative State

**Last updated: Saturday, August 29, 2026 (evening). Application APPROVED in
writing; lease signing Monday, Aug 31, 2:30 PM.** This file supersedes all
earlier apartment rankings, tour plans, and listing information. The apartment
search is **CLOSED**.

> **Privacy note.** This repository is public. This file deliberately omits
> salary figures, application documents, receipts, and any itemized list of
> valuables. Review before merging to `main` (which publishes to the live
> Pages site); move to private storage if even this level of detail should
> not be public.

---

## 1. The decision — FINAL

| Fact | Value |
|---|---|
| Community | Ambassador II Apartments (Fidelity Management Corporation) |
| Address | **851 Red Lion Road, Philadelphia, PA** |
| Unit | **D28** |
| Floor plan | Chelsea — 1 bedroom / 1 bathroom |
| Placement | First floor, private patio |
| Base rent | **$1,691 / month** |
| Extras | Upgraded bathroom package (offered for applying sight unseen — must be confirmed as $0 in the lease) |
| Laundry | Shared, directly outside the apartment door — $2 wash / $2 dry ($4 per full load) |
| Why first floor matters | Easy indoor storage of the heavy e-bike |

**Address correction, standing — now explained:** some flyers/online sources
list Ambassador II as **828** Red Lion Road. The Aug 29 approval letter
resolves the discrepancy: **828 Red Lion Road is the leasing office**; the
apartments (and unit D28) are at **851**. Use 851 as the unit's working
address, and 828 for the office. Final authority is the address **exactly as
written in the executed lease** — mover, insurance, utility, and legal
documents must reproduce that lease address verbatim once it exists.

Do not restart comparisons, revive eliminated properties, schedule tours, or
contact other leasing offices **unless** D28 is denied or the written lease
materially differs from the terms above.

## 2. Application status (as of Sat Aug 29, 2026, evening)

- Requested paperwork: **submitted** (including income documents).
- $250 security deposit: **paid**, receipt saved.
- Nova Credit income verification: ✅ **completed** (Sat Aug 29, ahead of the
  48-hour deadline).
- Application: ✅ **APPROVED in writing** — approval letter from Fidelity
  Management Corp. Credit Department, Sat Aug 29, 11:46 AM ET, plus a
  confirming phone call from the leasing office the same day.
- **Signing appointment: Monday, Aug 31, 2:30 PM** at the leasing office
  (per the approval letter, the office is 828 Red Lion Road).
- **72-hour clause:** the approval letter requires the lease to be **signed
  and funded within 72 hours** of receipt → deadline ≈ **Tuesday, Sept 1,
  11:46 AM ET**. The Monday appointment is safely inside it.
- Due at signing (per the phone call): **first month's rent + $1,490**, in
  cash or certified funds; bank stop planned beforehand. The **$250 already
  paid is credited toward the first month's rent** (confirmed by the leasing
  office), so the estimated total is **$1,441 + $1,490 ≈ $2,931** — confirm
  the exact figure and payee before the bank stop (questions below).
- Fully executed lease + key date: **not yet in hand** — expected Monday.
  The approval letter states the lease is not binding until executed by both
  parties **and a duplicate is delivered to the tenant** — do not leave the
  signing without a complete countersigned copy.

### Pre-signing questions for the leasing office (before Monday 2:30 PM; answers in writing where possible)

1. Email the lease and an **itemized move-in statement** in advance for
   review.
2. Exact **payee name** for the certified check(s), whether one or two
   checks, and the **exact amounts**.
3. What does the **$1,490** cover — security deposit, fees, or both? (The
   $250 credit toward first month's rent is confirmed by phone; verify it
   appears that way on the move-in statement, and confirm the total due
   Monday to the dollar — estimated ≈ $2,931.)
4. **Key/possession date**, lease start date, and any proration.
5. Is **proof of renter's insurance** required at signing, and with what
   coverage and interested-party wording?

### Monday, Aug 31 itinerary

Drive up from Wilmington, DE (Ian + mother). The job start has moved to
**Tuesday, Sept 1** at the employer's request (Monday is paid), so Monday is
fully clear for these errands.

1. Bank stop — certified funds (exact amount and payee confirmed first).
2. **2:30 PM — lease signing**, Ambassador leasing office.
3. **3:00 PM — fingerprinting appointment, 1900 Grant Ave** (employer
   clearances).
4. If keys are handed over Monday (hoped, not yet promised): photograph the
   unit's condition **before anything goes in**, take doorway and room
   measurements, then drop off the bags of clothes and essentials brought
   in the car.

⚠️ Flags: (a) the 2:30 → 3:00 gap leaves ~20 minutes for a signing that
normally takes longer — ask to start earlier or confirm the fingerprinting
site tolerates a late arrival, and do the bag drop-off and photos **after**
fingerprinting, not between; (b) the 72-hour funding deadline (~Tue 11:46
AM ET) collides with the first day of work Tuesday — everything must
complete Monday.

Bring: photo ID, certified funds, the $250 receipt, checkbook, this
checklist (Section 4B), the drop-off bags, and a phone for photos and
measurements. Ask the consultant to confirm key-handover timing — the
approval letter says they'll explain "how and when to pick up keys."

### Resolved

- Nova Credit income verification — completed Sat Aug 29, before the Sunday
  deadline (was flagged URGENT in the previous revision of this file). The
  email was verified authentic via its SPF/DKIM/DMARC headers, and Fidelity
  Management's role as Ambassador's manager was confirmed via public
  listings, before completion.
- Brookmont tour (Sat Aug 29, 10:30 AM) — moot; search closed.

## 3. Guardrails — in force until written approval + countersigned lease + confirmed possession date

- ❌ No further nonrefundable charges.
- ❌ Do not cancel temporary lodging or the Amsterdam apartment.
- ❌ Do not book a nonrefundable mover.
- ❌ Do not purchase apartment-specific furniture.
- ❌ No external actions, messages, or contact with anyone on Ian's behalf
  without explicit authorization.

## 4. Approval-to-lease checklist

### A. Now → signing

- [x] Complete Nova Credit income verification (done Sat Aug 29)
- [x] Brookmont tour (moot — search closed)
- [x] Receive written application decision (approval letter, Sat Aug 29)
- [ ] Get the five pre-signing questions (Section 2) answered before Monday 2:30 PM
- [ ] Sign and fund within the 72-hour window (by ~Tue Sept 1, 11:46 AM ET)
- [ ] If terms at signing differ materially from Section 1: pause and audit against Section 4B before signing; if the deal collapses, recover/clarify the $250 per its written classification before waking the dormant contingencies

### B. When approval / lease documents arrive — verify line by line, in writing

- [ ] 1. Ambassador II, 851 Red Lion Road, Unit D28 (exact legal address as the lease writes it)
- [ ] 2. Chelsea 1BR/1BA layout
- [ ] 3. First-floor placement and private patio
- [ ] 4. Base rent $1,691
- [ ] 5. Exact lease start, end, possession, and key dates
- [ ] 6. Upgraded bathroom package adds **no** rent or fee
- [ ] 7. How the $250 is classified, credited, and handled on denial or changed terms
- [ ] 8. Every mandatory monthly charge and the complete recurring monthly total
- [ ] 9. Exact cash due before keys, including the proration formula
- [ ] 10. Gas / electric / water / sewer / trash / internet responsibility
- [ ] 11. Laundry access and the $2 wash / $2 dry pricing
- [ ] 12. Renter-insurance requirements
- [ ] 13. Written permission to store the bicycle inside D28
- [ ] 14. Moving hours, truck location, Certificate of Insurance requirements, and whether movers may use the patio entrance
- [ ] 15. Parking, pool, package, patio, and community rules

Audit protocol: when the documents arrive, they get read line by line against
this list; every mismatch gets raised in writing with the leasing office
before signing. Nothing is signed with an unresolved mismatch on items 1–9.

### C. After countersigned lease + confirmed key date

- [ ] Renter's insurance bound effective on/before possession (per item 12)
- [ ] Utilities in name per item 10, effective on possession date
- [ ] Photograph and measure D28: doorways, walls, outlets, circuits, thermal, room dimensions
- [ ] Only then: update the professional moving inventory and book Stage 2
- [ ] Only then: end temporary lodging / Amsterdam apartment per their own notice terms

## 5. Dormant contingencies

Chapelcroft, 1700 Grant Avenue, and Brookmont remain **dormant** backups only.
No contact, no tours, no re-ranking unless D28 is denied or its written lease
materially differs from Section 1.

## 6. Two-stage move (framework locked; execution deferred)

- **Stage 1 — essentials under personal control**, after keys are confirmed:
  documents, medications, work clothing and RF field kit, e-bike + gear,
  critical computers/drives/one network setup, phone + encrypted backup,
  saxophone and irreplaceables, first-night household basics. All computers,
  drives, and high-value items travel under Ian's control — never in a general
  mover shipment.
- **Stage 2 — professional household move**, only after D28 is photographed
  and measured: furniture, rugs, the seven-foot display stand, safe, ~400 lb
  of gym equipment, and remaining household goods per the (separately held)
  inventory. Because Ian does not drive and the load includes heavy weights, a
  safe, and fragile glass furniture: compare a full-service interstate small
  move vs. a portable container with professional labor at both ends; verify
  any interstate mover through FMCSA; require a written survey-based estimate.
- Transportation for Stage 1 relies on Ian's mother, another licensed driver,
  delivery service, or a professional mover — Ian does not drive.

## 7. D28 setup direction (Chelsea layout — plan only, no purchases yet)

- **Entry / bike zone:** clear indoor route, washable mat, freestanding stand.
- **Living room:** modular couch, TV, one properly sized rug.
- **Interior technology wall:** desk, RF equipment, eventual ASGARDR rack —
  away from patio glass, plumbing, and direct sunlight.
- **Bedroom:** quiet, server-free.
- **Flex zone:** music equipment + compact exercise area.
- **Patio:** seating and plants only after community rules confirmed; never
  computers, drives, or permanent technology storage.
- **Laundry station:** hamper and supplies by the door (laundry room is
  directly outside D28).

Hold the full 400-lb gym setup, the seven-foot display stand, the second
large rug, and the complete 12U rack until doorway, wall, outlet, circuit,
thermal, and room measurements are complete.

## 8. Relocation baseline (context)

Moving from Amsterdam, NY to Northeast Philadelphia for the Senior DAS Field
Engineer role at R.F. Design & Integration (2180 Hornig Road), starting
**Tuesday, September 1, 2026** — moved from Aug 31 at the employer's
request, with Monday paid — under the signed REV2 offer (8/18). Income
comfortably clears the standard 3× rent screen for a $1,691 base rent.
